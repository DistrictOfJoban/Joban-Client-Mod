...
public abstract class AbstractListItem implements RenderHelper, GuiHelper {
    ...
    protected void drawBackground(GraphicsHolder graphicsHolder, GuiDrawing guiDrawing, int entryX, int entryY, int width, int mouseX, int mouseY, boolean widgetVisible, double elapsed, float tickDelta) {
        ...
        boolean entryHovered = inRectangle(mouseX, mouseY, entryX, entryY, width, getHeight());
        hoverOpacity = entryHovered ? Math.min(1, hoverOpacity + highlightFadeSpeed) : Math.max(0, hoverOpacity - highlightFadeSpeed);

        if(hoverOpacity > 0) drawListEntryHighlight(guiDrawing, entryX, entryY, width, getHeight());
    }
