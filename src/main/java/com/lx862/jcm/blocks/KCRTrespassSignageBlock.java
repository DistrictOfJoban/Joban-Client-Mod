...
public class KCRTrespassSignageBlock extends HorizontalWallAttachedMultiBlock {
    ...
    public VoxelShape getOutlineShape2(BlockState state, BlockView view, BlockPos pos, ShapeContext context) {
        switch(BlockUtil.getStateProperty(state, PART)) {
            case 0:
                return VoxelUtil.getDirectionalShape16(state.get(new Property<>(FACING.data)), 4, 2, 0, 16, 14, 0.1);
            case 1:
                return VoxelUtil.getDirectionalShape16(state.get(new Property<>(FACING.data)), 0, 2, 0, 12, 14, 0.1);
            default:
