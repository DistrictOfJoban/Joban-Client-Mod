...
public class WRLStationCeilingBlock extends CeilingGroundAttachedDirectionalMultiBlock {
    ...
    public VoxelShape getOutlineShape2(BlockState state, BlockView view, BlockPos pos, ShapeContext context) {
        switch(BlockUtil.getStateProperty(state, PART)) {
            case 0:
                VoxelShape vx1 = VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 0.5, 8, 1, 16, 9, 15);
                VoxelShape vx2 = VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 5.5, 9, 7.5, 6.5, 16, 8.5);
                return VoxelShapes.union(vx1, vx2);
            default:
                VoxelShape vx1R = VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 0, 8, 1, 15.5, 9, 15);
                VoxelShape vx2R = VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 10.5, 9, 7.5, 11.5, 16, 8.5);
