...
public class WRLStationCeilingPole extends PoleBlock {
    ...
    public VoxelShape getOutlineShape2(BlockState state, BlockView view, BlockPos pos, ShapeContext context) {
        switch(BlockUtil.getStateProperty(state, PART)) {
            case 0:
                return VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 5.5, 0, 7.5, 6.5, 16, 8.5);
            case 1:
                return VoxelUtil.getDirectionalShape16(BlockUtil.getStateProperty(state, FACING).data, 10.5, 0, 7.5, 11.5, 16, 8.5);
            default:
    ...
    public BlockState getPlacementState2(ItemPlacementContext ctx) {
        ...
        if((blockBelow.getBlock().data instanceof WRLStationCeilingPole) || blockIsAllowed(blockBelow.getBlock())) {
            return super.getPlacementState2(ctx).with(new Property<>(PART.data), BlockUtil.getStateProperty(blockBelow, PART)).with(new Property<>(FACING.data), BlockUtil.getStateProperty(blockBelow, FACING).data);
        } else {
            return null;
        }
    }
